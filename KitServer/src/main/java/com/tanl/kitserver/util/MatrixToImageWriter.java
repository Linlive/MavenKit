...
/**
 * copy from git hub ZXing
 * for create two dimension img
...
public class MatrixToImageWriter {
	...
	public static BufferedImage toBufferedImage(BitMatrix matrix, MatrixToImageConfig config) {
		...
		BufferedImage image = new BufferedImage(width, height,BufferedImage.TYPE_4BYTE_ABGR_PRE);//, config.getBufferedImageColorModel());
	 ...
	 * @param format image format
	 * @param stream {@link OutputStream} to write image to
	 * @param filePath logo icon path
	 * @throws IOException
	 */
	public static void writeToStream(BitMatrix matrix, String format, OutputStream stream, String filePath)
			throws IOException {
		BufferedImage image = toBufferedImage(matrix);
		//设置logo图标
		LogoConfig logoConfig = new LogoConfig();
		image = logoConfig.LogoMatrix(image, filePath);
